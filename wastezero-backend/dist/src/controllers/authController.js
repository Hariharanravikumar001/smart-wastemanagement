...
exports.googleLogin = exports.uploadProfileImage = exports.deleteAccount = exports.getAllUsers = exports.getUserStats = exports.getMe = exports.getUserById = exports.resetPassword = exports.verifyOtp = exports.forgotPassword = exports.changePassword = exports.updateProfile = exports.loginUser = exports.registerUser = void 0;
...
const getGoogleClient = () => {
    ...
    _googleClient = new google_auth_library_1.OAuth2Client(clientId || '770353118324-dummy.apps.googleusercontent.com', clientSecret);
    return _googleClient;
};
const registerUser = async (req, res) => {
    ...
    `;
        ...
        (0, emailService_1.sendEmail)(user.email, 'Welcome to WasteZero - Your Sustainable Journey Begins!', `Hello ${user.name}, welcome to WasteZero.`, welcomeHtml).catch(err => console.error(`[EMAIL_FAIL] Welcome email to ${user.email}:`, err.message));
...
const loginUser = async (req, res) => {
    try {
        ...
        jsonwebtoken_1.default.sign(payload, secret, { expiresIn: '5h' }, (err, token) => {
            if (err)
                throw err;
            res.json({
                token,
...
const updateProfile = async (req, res) => {
    try {
      ...
      `;
            // Notify NEW email
            (0, emailService_1.sendEmail)(email, 'WasteZero - Email Address Updated', `Your email address has been updated to ${email}.`, emailUpdateHtml);
            // Notify OLD email (for security)
            if (oldEmail) {
                (0, emailService_1.sendEmail)(oldEmail, 'WasteZero - Security Alert: Email Changed', `The email address for your account has been changed to ${email}.`, emailUpdateHtml);
            }
        }
        if (name)
            user.name = name;
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const emailSent = await (0, emailService_1.sendEmail)(user.email, subject, text, html);
        if (emailSent) {
...
const deleteAccount = async (req, res) => {
    try {
        ...
        // 1. Waste Requests
        await WasteRequest_1.default.deleteMany({
            $or: [{ citizenId: userId }, { volunteerId: userId }]
        });
        // 2. Applications
        await Application_1.default.deleteMany({ volunteer_id: userId });
        // 3. Messages
...
const googleLogin = async (req, res) => {
    try {
        ...
        const secret = process.env['JWT_SECRET'] || 'wastezero_secret_token';
        jsonwebtoken_1.default.sign(jwtPayload, secret, { expiresIn: '5h' }, (err, token) => {
            if (err)
                throw err;
            res.json({
                token,
