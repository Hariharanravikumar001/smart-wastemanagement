...
exports.updateApplicationStatus = exports.getVolunteerApplications = exports.getAdminApplications = exports.applyForOpportunity = void 0;
...
const applyForOpportunity = async (req, res) => {
    try {
        ...
        const opportunity = await Opportunity_1.default.findById(opportunity_id);
        if (!opportunity) {
...
exports.applyForOpportunity = applyForOpportunity;
...
const getAdminApplications = async (req, res) => {
    try {
        ...
        if (role === 'ngo') {
            const myOpportunities = await Opportunity_1.default.find({ ngo_id: req.user.id }).select('_id').lean();
            const oppIds = myOpportunities.map(opp => opp._id);
            query = { opportunity_id: { $in: oppIds } };
        }
        // Admin gets all (empty query)
        const applications = await Application_1.default.find(query)
...
const getVolunteerApplications = async (req, res) => {
    try {
        const applications = await Application_1.default.find({ volunteer_id: req.user.id })
            .populate({
            path: 'opportunity_id',
...
exports.getVolunteerApplications = getVolunteerApplications;
...
const updateApplicationStatus = async (req, res) => {
    try {
        ...
        const application = await Application_1.default.findById(appId)
        ...
        const opp = application.opportunity_id;
        if (!opp) {
        ...
        if (status === 'accepted' && opp) {
            ...
            await (0, notificationService_1.createNotification)(application.volunteer_id.toString(), 'Application Accepted', `Congratulations! Your application for "${opp.title}" has been accepted.`, 'success');
        }
        else if (status === 'rejected' && opp) {
            // Real-time Notification
            await (0, notificationService_1.createNotification)(application.volunteer_id.toString(), 'Application Update', `Your application for "${opp.title}" was not accepted at this time.`, 'info');
        }
